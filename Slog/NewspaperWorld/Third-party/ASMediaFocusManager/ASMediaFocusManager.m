...
@interface ASMediaFocusManager () <ASMediaFocusControllerDelegate>
...
- (ASMediaFocusController *)focusViewControllerForView:(UIView *)mediaView
{
    ...
    viewController.closeButtonTitle     = self.closeButtonTitle;
    viewController.isCloseButtonVisible = self.isCloseButtonVisible;
    viewController.delegate = self;

    if (!self.isCloseButtonVisible)
    {
...
- (void)handleFocusGesture:(UIGestureRecognizer *)gesture
{
    ...
    [UIView animateWithDuration:self.animationDuration
                     animations:^{
                         ...
                         focusViewController.closeButton.alpha = 1.0;
    ...
    if (self.focusViewController)
    {
        ...
        [UIView animateWithDuration:self.animationDuration
                         animations:^{
                             ...
                             self.focusViewController.closeButton.alpha = 0.0;
                             CGRect originRect = [_delegate mediaFocusManager:self startFrameforView:self.mediaView];
                             CGPoint originCenter = CGPointMake(originRect.origin.x + originRect.size.width/2,
                                                                originRect.origin.y + originRect.size.height/2);

                             contentView.center = originCenter;//[contentView.superview convertPoint:originCenter fromView:self.mediaView.superview];
                         ...
                         completion:^(BOOL finished) {
                             [UIView animateWithDuration:(self.elasticAnimation?self.animationDuration*kAnimateElasticDurationRatio:0)
                                              ...
                                              completion:^(BOOL finished2) {
                                                  ...
                                                  if ([self.focusViewController respondsToSelector:@selector(removeFromParentViewController)])
