                             ...
                             onError:(void (^)(NSError*)) errorBlock
{
    if(REVIEW_ALLOWED)
    {
        ...
        NSString *uniqueID = [dev uniqueDeviceIdentifier];

        // check udid and featureid with developer's server
		
        NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/%@", NWApiClientBaseURL, @"featureCheck.php"]];
                         ...
                         onError:(void (^)(NSError*)) errorBlock
{
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/%@", NWApiClientBaseURL, VERIFY_PRODUCT_PATH]];
    ...
    if (self.token)
        postData = [postData stringByAppendingFormat:@"&token=%@",      self.token];
    if (self.bookID != -1)
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    id json = [parser objectWithString:responseString error:&error];
	...
	if( ([responseMethod isEqualToString:NWApiMethodRegisterPurchase] && responseCode == kNWApiResponseCodeSuccess))
	{
